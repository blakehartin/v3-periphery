...
contract Quoter is IQuoter, IUniswapV3SwapCallback, PeripheryImmutableState {
    ...
    function uniswapV3SwapCallback(
        ...
        require(amount0Delta > 0 || amount1Delta > 0, 'delta invalid quoter callback'); // swaps entirely within 0-liquidity regions are not supported
        ...
        if (isExactInput) {
            ...
            if (amountOutCached != 0) require(amountReceived == amountOutCached, 'received cached not equal, quoter');
            assembly {
                let ptr := mload(0x40)
